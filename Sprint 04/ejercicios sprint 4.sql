...
-- Ejercicio 1
-- Realiza una subconsulta que muestre a todos los usuarios con más de 80 transacciones utilizando al menos 2 tablas.
SELECT u.id AS userId, u.name as nombre, transactions2.conteoTransacciones
FROM users u
JOIN (
    SELECT user_id, COUNT(id) AS conteoTransacciones
    FROM transactions t
    GROUP BY user_id
    HAVING COUNT(id) > 80
) transactions2 
ON u.id = transactions2.user_id;
...
select cc.iban, round(avg(t.amount),2) as mediaVentas
from companies c
join transactions t
